...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const SVG_ICONS = {
    ...
    'navigation': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="3 11 22 2 13 21 11 13 3 11"></polygon></svg>',
    ...
    'banknote': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="6" width="20" height="12" rx="2" ry="2"></rect><circle cx="12" cy="12" r="2"></circle><path d="M6 12h.01M18 12h.01"></path></svg>'
  };
  ...
  // Helper to classify a photo name into one of the 4 category tags
  function classifyPhotoTag(filename) {
    const name = filename.toLowerCase();
    if (name.includes("depan rumah")) return "depan-rumah";
    if (name.includes("ruang depan")) return "ruang-depan";
    if (name.includes("ruang dalam")) return "ruang-dalam";
    if (name.includes("kamar mandi")) return "kamar-mandi";
  ...
  function setupStaticData() {
    ...
    if (addressContainer) {
      addressContainer.innerHTML = `
        ${SVG_ICONS['map-pin']}
        <div>
          <p style="font-weight: 600; color: var(--text-primary); margin-bottom: 4px;">Kontrakan Rumah Tanah Baru</p>
  ...
  function setupSplashHero() {
    ...
    // Pick 4 dynamic photos to showcase
    const splashPhotoTags = ["depan-rumah", "ruang-depan", "ruang-dalam", "kamar-mandi"];
    const splashPhotos = [];
  ...
  function setupRentSchemeSelector() {
    ...
    function syncWhatsAppLink(schemeText) {
      const waButtons = document.querySelectorAll(".wa-contact-btn");
      const baseMsg = `Halo Pak Febri, saya tertarik untuk menyewa kontrakan rumah Tanah Baru dengan skema pembayaran [${schemeText}].`;
